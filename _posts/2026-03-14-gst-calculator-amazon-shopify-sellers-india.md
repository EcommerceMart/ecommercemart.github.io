---
title: "GST Calculator for Online Sellers India with Examples for Amazon & Shopify Sellers"
description: "Master GST effortlessly with our reliable GST calculator for online sellers India. Get practical examples for Amazon and Shopify to simplify tax computations."
author_profile: true
read_time: true
comments: true
share: true
...
<style>
  ...
  .gst-calculator-container input[type="checkbox"] {
    margin-right: 8px;
  }
  ...
  .gst-calculator-container .results p strong {
    color: #000;
  }
</style>

<div class="gst-calculator-container">
  <h4>Simple GST Calculator</h4>
  <div>
    <label for="salePrice">Selling Price (before GST):</label>
    <input type="number" id="salePrice" placeholder="e.g., 1000" min="0">
  </div>
  <div>
  ...
  <div>
    <input type="checkbox" id="interStateSale">
    <label for="interStateSale" style="display: inline;">Is this an inter-state sale?</label>
  </div>
  <button onclick="calculateGST()">Calculate GST</button>
  <div class="results" id="gstResults">
    <p><strong>Taxable Value:</strong> ₹<span id="outputTaxableValue">0.00</span></p>
    <p><strong>CGST:</strong> ₹<span id="outputCGST">0.00</span></p>
    <p><strong>SGST:</strong> ₹<span id="outputSGST">0.00</span></p>
    <p><strong>IGST:</strong> ₹<span id="outputIGST">0.00</span></p>
    <p><strong>Total GST Amount:</strong> ₹<span id="outputTotalGST">0.00</span></p>
    <p><strong>Total Price (with GST):</strong> ₹<span id="outputTotalPrice">0.00</span></p>
  </div>
</div>
...
<script>
  function calculateGST() {
    ...
    const isInterState = document.getElementById('interStateSale').checked;
    ...
    const totalGSTAmount = salePrice * (gstRate / 100);
    const totalPrice = salePrice + totalGSTAmount;
    ...
    document.getElementById('outputTaxableValue').textContent = salePrice.toFixed(2);
    document.getElementById('outputCGST').textContent = cgst.toFixed(2);
    document.getElementById('outputSGST').textContent = sgst.toFixed(2);
    document.getElementById('outputIGST').textContent = igst.toFixed(2);
    document.getElementById('outputTotalGST').textContent = totalGSTAmount.toFixed(2);
    document.getElementById('outputTotalPrice').textContent = totalPrice.toFixed(2);
  }

  // Initial calculation when the page loads, if default values are set.
  document.addEventListener('DOMContentLoaded', calculateGST);
  document.getElementById('salePrice').value = 1000; // Default example value
</script>
...
Total IGST for this order = ₹108 + ₹24 = ₹132.
Total selling price = ₹600 + ₹200 = ₹800.
Total price with GST = ₹800 + ₹132 = ₹932.
...
**Solution:**
*   **Official Sources:** Always refer to the official GST portal ([gst.gov.in](https://www.gst.gov.in/)) for the latest HSN/SAC codes and their GST rates.
...
*   **Official GST Portal:** The best place to find HSN/SAC codes is the official GST portal ([gst.gov.in](https://www.gst.gov.in/)). They have a search functionality.
...
*   **Stay Informed:** Regularly visit the official GST portal ([gst.gov.in](https://www.gst.gov.in/)) for announcements, circulars, and notifications.
...
You need to find the correct HSN (Harmonized System of Nomenclature) code for your product. You can find this on the official GST portal ([gst.gov.in](https://www.gst.gov.in/)) or by checking your supplier's invoices.
